...
router.get("/country", (req, res) => {
  ...
  if (!GNP2){
    const countrySelectWhere = "SELECT * FROM country WHERE GNP <= ? ORDER BY GNP asc";
    mysql.execute(countrySelectWhere, [GNP1], (err, result, f) => {
    res.json(result);
  });
  } else {
    const countrySelectWhere = "SELECT * FROM country WHERE GNP BETWEEN ? AND ? ORDER BY GNP asc";
    mysql.execute(countrySelectWhere, [GNP1, GNP2], (err, result, f) => {
    res.json(result);
  });
  }
});
