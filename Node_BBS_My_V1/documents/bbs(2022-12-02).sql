...
INSERT 를 수행할때 특별히 값이 없을때 또는 0 일때
	기존의 칼럼 값을 비교하여 1만큼 추가해서 채워넣는 속성
DEFAULT 속성
...
tbl_bbs 에 키값이 있고, 그 키에 해달하는 연결 정보가 tbl_files 에 있는 경우
해당하는 데이터는 삭제, 키값의 Update 는 기본적으로 금지된다
ON DELETE CASCADE
...
-- 단 이때는 두 table 간의 EQ JOIN 이 성립되어야 한다
