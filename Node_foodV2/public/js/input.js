  ...
  tTable?.addEventListener("click", (tag) => {
    ...
    if (target.tagName === "TD") {
      ...
      for (const [index, td] of tds.entries()) {
         ...
         * index 는 0 ~ 5까지 이다
         * input box 는 모두 5개 이므로
         * input 의 index 는 0 ~ 4 까지 이다
         *
         * tInputs[index] 값을 검사하여
         * 이 값이 없으면(nudefubed)이면 제외하기
         */
        if (tinputs[index]) {
      ...
      // input tag 를 하나 생성하고,
      // input tag 속성을 inputProps 객체에 설정된 값으로 채워라
      ...
      // 임의로 <input name="t_seq" value="PK값" style="display=hidden"> tag fmf 만들어
      // 화면에 form에 추가하기
      // 그래야만 router(server)로 데이터를 전송했을때 t_saq 를 함께 보내서
      //
