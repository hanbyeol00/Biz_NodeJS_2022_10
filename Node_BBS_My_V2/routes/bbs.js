 ...
 *
 * 이러한 경우를 방지하기 위하여
 * express 에서는 res... 코드는 반드시 return 문을 붙여야한다
 */
router.get("/", async (req, res) => {
  try {
    const result = await BBS.findAll({ include: "m_files" });
    res.render("bbs/list", { bbsList: result });
  } catch (err) {
    console.error(err);
 ...
 * input type = 'file' 속성을 찾는 tag 들이 form 에 여러개 있을때
 * 한번에 받기
